...
test('parse commas', () => {
  ...
  const expected = new Amount([new SingleAmount(false, '', 1_000_000_000, -3)])
  expect(actual).toStrictEqual(expected)
})
...
test('plus non matching prefix', () => {
  ...
  {
    const actual = left.plus(right)
    const expected = new Amount([
      new SingleAmount(false, '$', 1, 0),
      new SingleAmount(false, 'USD', 2, 0),
    ])
    expect(actual).toStrictEqual(expected)
  }
  {
    const actual = right.plus(left)
    const expected = new Amount([
      new SingleAmount(false, 'USD', 2, 0),
      new SingleAmount(false, '$', 1, 0),
    ])
  ...
  {
    const actual = left.plus(right).plus(left.negate())
    const expected = new Amount([
